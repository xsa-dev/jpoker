...
public class Main {
        ...
        for (int number = 0; number < imgs.length; number++) {
            ...
            switch (cardCollor) {
                case Black:
                    ...
                    secondLayerColor = CardCollors.get(imgs[number].getRGB(secondLayerPoint.x, secondLayerPoint.y));
    ...
    @Deprecated
    private static boolean whiteFinder(BufferedImage[] imgs, int number, enumCardColors cardCollor) {
        boolean b = cardCollor.equals(enumCardColors.Black) || cardCollor.equals(enumCardColors.Red);
        if (b) {
            int white = imgs[number].getRGB(45, 30);
